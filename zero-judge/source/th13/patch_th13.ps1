...
$originalSha256 = "422F40AA176C9AAB7D0178CE6033802B4143921A5E0E463EF99259EF6D100A58"
$patchedSha256 = "17A3634B582238BE53AB9A378C2148AA462FEB30D33800F964481F0628767A44"
$hitboxOffset = 0xBF1A8
...
try {
    ...
    if ($Action -eq "Apply") {
        ...
        if ($currentHash -ne $originalSha256) {
            throw "Unsupported th13.exe. Expected the original Japanese 1.00c executable. Current SHA-256: $currentHash"
        }

        $data = [IO.File]::ReadAllBytes($resolvedExe)
        if (-not (Test-Bytes $data $hitboxOffset $originalHitboxes)) {
            throw "Original hitbox bytes were not found at file offset 0xBF1A8."
        }

        if (Test-Path -LiteralPath $backupPath) {
            $backupHash = Get-Sha256 $backupPath
            if ($backupHash -ne $originalSha256) {
                throw "A backup file already exists but is not an original Japanese 1.00c executable: $backupPath"
            }
        }
        else {
        ...
        Write-Host "Reimu, Marisa, Sanae, and Youmu hitbox radii are now 0.0."
        Write-Host "Backup: $backupPath"
    }
    else {
        if ($currentHash -eq $originalSha256) {
            Write-Host "The executable is already the original Japanese 1.00c version."
            exit 0
        }
        ...
        if (-not (Test-Bytes $data $hitboxOffset $zeroHitboxes)) {
            throw "Zero-hitbox bytes were not found at file offset 0xBF1A8."
        }
