...
$originalSha256 = "C11776019F083978E66027E7394DAFB1FB9543AFCA986F28049A49417E341929"
$patchedSha256 = "ADFEA780BEFA46BA9AC18CD213BF98191C55824DECA7624DC03124E2F64ABDA3"
$hitboxOffset = 0x91E98
...
try {
    ...
    if ($Action -eq "Apply") {
        ...
        if ($currentHash -ne $originalSha256) {
            throw "Unsupported th16.exe. Expected the original Japanese 1.00a executable. Current SHA-256: $currentHash"
        }

        $data = [IO.File]::ReadAllBytes($resolvedExe)
        if (-not (Test-Bytes $data $hitboxOffset $originalHitboxes)) {
            throw "Original hitbox bytes were not found at file offset 0x91E98."
        }
        ...
        Write-Host "Reimu, Cirno, Aya, and Marisa hitbox radii are now 0.0."
        ...
        if (-not (Test-Bytes $data $hitboxOffset $zeroHitboxes)) {
            throw "Zero-hitbox bytes were not found at file offset 0x91E98."
        }
